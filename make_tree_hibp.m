...
if num_MIBPs_left > 0
    ...
    cluster0 = [split_cluster '0'];
    cluster0_mass = B_get_probmass2(cluster0,energy)
    num_conf_splits = 0;
    % Get rid of while loop if you only want 1 conflicting split
    pairs = mibp;
    while cluster0_mass > min_cluster
        disp(['trying to conflict'])
        pairs
        disp(['in ' cluster0])
        if num_conf_splits > max_conf_splits
            disp('too many conf splits')
            break
        end
        num_conf_splits = num_conf_splits + 1;
        [conflicting_bp,conflicting_prob] = get_conflicting_of_pairs(pairs,cluster0);
        pairs = [pairs;conflicting_bp];
        if conflicting_prob*cluster0_mass < min_cluster
            disp('not getting enough back from conflicting')
            break
        end
        [tree_strings,all_tree_strings] = full_split(cluster0,tree_strings,all_tree_strings,conflicting_bp);
        cluster0 = [cluster0 '0'];
        cluster0_mass = B_get_probmass2(cluster0,energy)
    end
    
    [tree_strings,all_tree_strings] = add_hibp(tree_strings,all_tree_strings);
    
    [tree_strings,all_tree_strings] = make_tree_hibp(tree_strings, all_tree_strings, num_MIBPs_left - 1,mi_cutoff, energy); % recursively call B_make_trees with one less mibp
end
end
