...
conflicting = (bp_probs(:,1) < pairs(1,1) & bp_probs(:,2) > pairs(1,1) & bp_probs(:,2) < pairs(1,2)) | (bp_probs(:,1) > pairs(1,1) & bp_probs(:,1) < pairs(1,2) & bp_probs(:,2) > pairs(1,2)) | (bp_probs(:,1) == pairs(1,1) & bp_probs(:,2) ~= pairs(1,2)) | bp_probs(:,1) == pairs(1,2) | bp_probs(:,2) == pairs(1,1) | (bp_probs(:,1) ~= pairs(1,1) & bp_probs(:,2) == pairs(1,2));
for i = 2:size(pairs,1)
    conflicting = conflicting & ((bp_probs(:,1) < pairs(i,1) & bp_probs(:,2) > pairs(i,1) & bp_probs(:,2) < pairs(i,2)) | (bp_probs(:,1) > pairs(i,1) & bp_probs(:,1) < pairs(i,2) & bp_probs(:,2) > pairs(i,2)) | (bp_probs(:,1) == pairs(i,1) & bp_probs(:,2) ~= pairs(i,2)) | bp_probs(:,1) == pairs(i,2) | bp_probs(:,2) == pairs(i,1) | (bp_probs(:,1) ~= pairs(i,1) & bp_probs(:,2) == pairs(i,2)));
end
